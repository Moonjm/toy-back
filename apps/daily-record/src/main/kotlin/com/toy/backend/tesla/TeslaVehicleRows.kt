 ...
 * 영원히 남는다 — 리포지토리 SQL이 24시간 범위를 거는 이유다.
 */
...
/**
 * `states`의 한 구간. **범위에 맞춰 이미 잘려서 온다** — 자르는 규칙을 서버 한 곳에만 두려는
 * 것이다(앱이 범위 밖 값을 받아 스스로 자르면 규칙이 두 곳에 생긴다).
 *
 ...
 *
 * **마감되지 않은 유령 세션은 여기 오지 않는다.** 리포지토리 SQL의 24시간 범위가 거른다
 * (`ActivityRow`와 같은 규칙이다). 진행 중인 진짜 세션은 `toUtc`가 범위 끝으로 막혀 온다.
 */
