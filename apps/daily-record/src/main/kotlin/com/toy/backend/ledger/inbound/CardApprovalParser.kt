...
class CardApprovalParser : MessageParser {
    ...
    companion object {
         ...
         * 여기 걸리지 않는다.
         *
         * **앞에 경계를 요구한다** — 줄 시작, 공백, 또는 「카드」다. 이것이 없으면 `미승인`처럼
         * 「승인」을 접미사로 품은 낱말이 걸린다. 승인 거절 문자가 금액·일시 줄을 정상으로 달고
         * 오면 **거절된 금액이 지출로 저장된다.** 낱말 경계(`\b`)로는 못 막는다 — 자바 정규식의
         * `\b`는 `\w`(=`[a-zA-Z_0-9]`) 기준이라 한글 사이에서 경계를 찾지 못한다.
         *
         * **「카드」를 경계로 함께 인정하는 이유**는 옛 패턴 `카드\s*(승인|취소)`가 공백을
         * 요구하지 않아 `대한항공카드승인`처럼 붙여 쓴 형태를 받았기 때문이다. 공백만 경계로
         * 삼으면 그 형태가 조용히 `PARSE_FAILED`가 된다 — 되던 것이 안 되는 회귀다.
         * `대한항공카드미승인`은 「승인」 앞이 「미」라 여전히 걸리지 않는다.
         */
        private val KIND_LINE_REGEX = Regex("""(?:^|\s|카드)(승인|취소)$""")
