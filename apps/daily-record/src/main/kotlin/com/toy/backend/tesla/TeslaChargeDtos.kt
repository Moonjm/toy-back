 ...
 *
 * `totalCount`는 `limit`과 무관하되 **같은 한 달 범위 안의** 개수다. 앱이 배지에 띄우고
...
/**
 * **널 불가 타입이라 금액을 비울 수 없다** — 되돌리기를 두지 않기로 한 결정의 표현이다.
 * 값이 빠지거나 null로 오면 Jackson이 객체를 만들다 실패해 400이 나간다.
 *
...
data class TeslaChargeTotalsResponse(
     ...
     * 그쪽은 최근 한 달 범위가고 이쪽은 전 기간이다. 앱이 두 값을 같은 배지로 쓰면 어긋난다.
     *
