...
class DispatchVisionClient(
    ...
    private fun prompt(
        ...
        val rowInstruction =
            if (targetName != null) {
                // **성명 컬럼이 보인다고 단정하지 않는다.** hasNameColumn은 「보이는가」를 묻는
                // 값인데 프롬프트가 답을 정해 주면, 잘린 사진에도 모델이 true로 답하고 아무 행이나
                // 읽어 온다. 그러면 저장된 행 위치를 쓰는 갈래도 ROSTER_NOT_FOUND 거부도 건너뛰어
                // 다른 기사의 근무가 그대로 들어온다 — 설계가 세운 안전장치가 통째로 우회된다.
                """
                **먼저 이 사진에 성명(기사 이름) 컬럼이 보이는지 판단하라.**
        ...
        return """
            ...
            표 제목이 잘려 보이지 않거나 읽을 수 없으면 year와 month에 0을 넣어라.
            추측해서 채우지 마라 — 지어낸 연월은 다른 달의 줄 위치를 불러와 다른 기사의 근무를 읽는다.
