...
data class InsightsMonth(
     ...
     *
     * **월 경계를 걸치는 세션은 시작한 달로 친다** — 걸치는 것이 전 기간 충전 9건·주행 1건,
     * 합쳐 3,220분(달당 약 0.12%)뿐이고, 쪼개면 `/tesla/summary`의 `trend`(시작 시각 기준)와
     * 같은 달에 다른 숫자를 내게 된다. 요일별은 걸치는 비율이 64%라 거기서만 쪼갠다.
     */
    val idleMin: Int,
...
data class InsightsWeekday(
     ...
     * 최대 하루치 부풀어 오른다.
     *
     * 충전은 KST 자정으로 쪼개 넣고(전 기간 64%가 자정을 걸친다) 주행은 시작 요일로 친다
     * (5년에 9건·127분뿐이라 자릿수가 다르다).
     */
    val idleMin: Int,
)
 ...
 * (이유는 `TeslaInsightsService.PARK_DRAIN_DAYS` 참고).
 *
