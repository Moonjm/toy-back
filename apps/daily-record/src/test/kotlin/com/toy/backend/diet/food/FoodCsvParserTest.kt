...
class FoodCsvParserTest :
    BehaviorSpec({
        ...
        Given("정상 행") {
            When("한 줄을 파싱하면") {
                val foods = FoodCsvParser.parse(sequenceOf(header, "D000001,300,180.5,12.3,15.1,8.2,제육볶음"), FoodDataset.DISH).toList()
        ...
        Given("이름에 쉼표가 든 행") {
            When("파싱하면") {
                val foods = FoodCsvParser.parse(sequenceOf(header, "D000002,200,100,10,5,2,밥, 국"), FoodDataset.DISH).toList()
        ...
        Given("1인분 기준량이 비어 있는 행") {
            When("파싱하면") {
                val foods = FoodCsvParser.parse(sequenceOf(header, "D000003,,150,20,10,3,김치찌개"), FoodDataset.DISH).toList()
