...
class FoodCsvParserTest :
    BehaviorSpec({
        ...
        Given("망가진 행") {
            ...
            When("estimatedFields 컬럼이 없던 14컬럼 옛 정제본이면") {
                val foods =
                    FoodCsvParser
        ...
        Given("브랜드가 빈 행") {
            When("파싱하면") {
                ...
                Then("추정 표시도 null이다 — 전부 원본 값이라는 뜻이다") {
                    foods[0].estimatedFields shouldBe null
                }
            }
        }

        // 프랜차이즈 표는 열량·단백질·나트륨·당류·포화지방만 싣는다. 탄수화물·지방은 잔여
        // 열량에서 계산한 값이라, 이 표시가 없으면 한 행 안에서 공식값과 추정값이 똑같아 보인다.
        Given("추정으로 채운 행") {
            When("파이프로 이어진 필드 이름이 오면") {
                val foods =
                    FoodCsvParser
                        .parse(
                            sequenceOf(header, "D000011,263,217,18.4,8.7,12.1,5.7,304,0,4.5,0,0,carbs|fat,롯데리아,한우불고기버거"),
                            FoodDataset.DISH,
                        ).toList()

                // 파일 구분자가 쉼표라 CSV에서는 파이프로 쓰고, 저장은 쉼표로 한다.
                Then("쉼표로 바꿔 담는다") {
                    foods[0].estimatedFields shouldBe "carbs,fat"
                }
            }
            ...
            When("한쪽만 추정한 행이면") {
                val foods =
                    FoodCsvParser
                        .parse(
                            sequenceOf(header, "D000012,200,150,20,10,3,1,300,1,0.5,0,20,fat,,된장국"),
                            FoodDataset.DISH,
                        ).toList()

                // 탄수화물만 비어 있는 행이 647건, 지방만 비어 있는 행이 105건 있다. 원본에
                // 있던 값까지 추정으로 표시하면 표시가 무의미해진다.
                Then("채운 것만 담는다") {
                    foods[0].estimatedFields shouldBe "fat"
                }
            }

            When("모르는 필드 이름이 섞여 오면") {
                val foods =
                    FoodCsvParser
                        .parse(
                            sequenceOf(header, "D000013,200,150,20,10,3,1,300,1,0.5,0,20,carbz|fat,,비빔밥"),
                            FoodDataset.DISH,
                        ).toList()

                // 오타 하나로 멀쩡한 영양소까지 잃으면 손해가 크다 — 이 값은 화면 배지 하나를
                // 좌우할 뿐이다.
                Then("그것만 버리고 행은 살린다") {
                    foods.size shouldBe 1
                    foods[0].estimatedFields shouldBe "fat"
                    foods[0].kcalPer100g shouldBe 150.0
                }
            }
            ...
            When("아는 이름이 하나도 없으면") {
                val foods =
                    FoodCsvParser
                        .parse(
                            sequenceOf(header, "D000014,200,150,20,10,3,1,300,1,0.5,0,20,carbz,,비빔밥"),
                            FoodDataset.DISH,
                        ).toList()

                Then("빈 문자열이 아니라 null이다 — 「추정 없음」과 같은 취급이다") {
                    foods[0].estimatedFields shouldBe null
                }
            }
        }
