 ...
 */
class DailyDietFeedbackRepositoryQueryTest :
    BehaviorSpec({
        Given("파생 쿼리 이름들은") {
            val derived =
                DailyDietFeedbackRepository::class.java.declaredMethods
            ...
            Then("하나도 빠짐없이 파서를 통과한다 — 못 통과하면 기동 시점에 앱이 안 뜬다") {
                ...
                derived.forEach {
                    withClue("${it.name} 가 파싱되지 않는다") {
                        shouldNotThrowAny { PartTree(it.name, DailyDietFeedback::class.java) }
                    }
                }
            }
        }
