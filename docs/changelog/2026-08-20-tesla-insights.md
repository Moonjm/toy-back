...
(약 750KB)이라 뒤집혔다. 솎으면 48시간 90개·168시간 440개 안팎이다 — 조회 범위가 「지금부터
N시간 전」으로 미끄러지는 롤링 값이라 잴 때마다 정확한 개수는 달라진다. 12,517행·750KB(그리고
상한 168시간의 102,141행·6MB)는 솎지 않으면 위험하다는 근거라 정밀도가 뜻을 갖지만, 솎은 뒤의
개수는 자릿수만 의미가 있다.
...
## 검증 방식 — 개발 중 손으로 한 일회성 확인이고, 저장소에 재현 수단이 없다
...
대신 커밋된 SQL 상수 열한 개를 그대로 뽑아 `:start`·`:end` 같은 바인드 파라미터만 실제 값으로
치환해, 개발 중 파이의 실제 TeslaMate DB에 `psql`로 손으로 돌렸다. 전부 정상 실행됐고 값이
계획 단계 실측과 일치했다 — 팬텀 드레인 표본 80·80·71, SoC 버킷이 1~10만 나오고 11번은 없음,
배터리 표본 48시간 90개 안팎, 기록 셋(`records`) 3619·3619·3342 등. 컬럼 이름·별칭·SQL 논리는
이 확인으로 검증됐다.

**이 확인은 자동 테스트가 아니고, 저장소 안에 재현할 스크립트나 결과가 남아 있지 않다.** 회귀를
막아 주지 않는다 — SQL을 고치면 같은 확인을 다시 손으로 해야 한다. 엔드포인트 실호출(HTTP
왕복, 토큰 발급, 전체 응답 크기·응답 시간)도 이번에 하지 않았고 앱이 실제로 배포된 뒤로 남는다.
