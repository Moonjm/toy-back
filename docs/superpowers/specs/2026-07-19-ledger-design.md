...
내역 저장은 별도 트랜잭션에서 수행한다 — 저장이 실패해도 rollback-only 오염 없이
`PARSE_FAILED` 원문 기록이 남아 재처리할 수 있다.

**동시성 방어는 의도적으로 두지 않는다.** 사용자 2명이 단축어로 순차 전송하고 스케줄러도
단일 인스턴스에서 하루 1회 도는 구조라, 락·선점·상태 선저장은 얻는 것보다 복잡도가 크다.
...
| `POST /inbound` | API 키 또는 JWT | `{ "text": "원문" }` — 파싱 후 저장/취소매칭하고 결과와 함께 원문 기록. 항상 수신 기록이 생성되므로 201 + `Location: /inbound/{id}/retry` 응답 |
