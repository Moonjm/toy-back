...
`dataset`(`DISH`/`RAW`/`PROCESSED`)은 매칭 규칙을 가르는 축이다 — 「식품DB」 절 참조.
...
`NutritionSource`(DB_MATCHED/LLM_ESTIMATED), `FoodDataset`(DISH/RAW/PROCESSED).
...
>
> 사진만으로 절대량을 맞히는 건 되는 문제가 아니다. 상용 앱들도 「1인분 조절」을 전면에 두고
...
예) 탄 75% / 단 8% / 지 17% → excess 10 + 2 + 0 = 12 → `100 − 12` = **88점**.
밥만 먹으면(탄 100/단 0/지 0) excess 35 + 10 + 15 = 60 → 40점.

> **2026-07-31 — 감점 기울기를 2.0에서 1.0으로 내렸다.** 튜닝이 아니라 이중 계산을 되돌린 것이다.
>
> 세 비율은 **합이 100%라서 서로 독립이 아니다.** 지방이 30%p 높으면 탄수화물은 필연적으로
> 그만큼 낮아지는데, 세 감점을 그냥 더하면 하나의 불균형을 두 번 센다. 음식DB 6,090종을 한
> 가지만 먹은 끼니로 재 보니 범위를 벗어난 5,764건 중 **89%가 두 개 이상의 매크로에서 동시에**
> 걸렸고, 감점 합이 가장 큰 감점의 평균 1.62배였다.
>
> 그래서 2.0에서는 **6,090종 중 1,506종(24.7%)이 0점**이었다. 치킨도 삼겹살도, 실제로 기록한
> 제육볶음+두부+잡곡밥 한 끼도 전부 0점이라 서로 구분되지 않았다. 1.0에서는 0점이 71종(1.2%),
> 중앙값이 49 → 74가 된다. 탄수화물이 아예 없는 순수 지방 같은 극단은 여전히 0점이다.
>
> **저장된 `Meal.score`는 옛 기준으로 계산된 값이지만 화면은 흔들리지 않는다** — 표시용 점수는
> `toResponse`가 매번 다시 계산하고, 통계도 `DietStatsService.dayScoreOf`가 다시 계산한다.
> 저장 컬럼은 피드백 프롬프트에만 쓰이고, 항목을 고치면 함께 갱신된다.
>
> **남는 한계** — 끼니 점수는 구성만 보고 양을 보지 않아서, 한 항목의 양이 잘못 인식되면
...
**출력 규격 (프롬프트에 강제)** — 3요소: ① 잘한 점 1개 ② 부족·과다 1개
③ **구체적 음식이 담긴 개선 행동 1개**. 2~3문장 한국어 존댓말. ③을 강제하지 않으면
"골고루 드세요"류로 흐른다. 의학적 진단·처방·특정 질환 언급은 금지 항목으로 명시한다.

> **2026-07-30 — ①만 「없으면 건너뛴다」로 열었다.**
> 실기동에서 튀긴 치킨을 먹은 날에 *"오늘 식단에서 튀긴 음식 대신 구운 닭 가슴살을 선택하신 점은
> 훌륭했습니다"* 가 나왔다. 칭찬할 것이 없는 식단에서 모델이 **먹지 않은 음식을 지어내** 실제와
> 정반대를 말한 것이다. 셋 다 강제하면 없는 장점을 만들어 채운다.
> 격려가 기록을 이어가게 만든다는 게 ①을 넣은 이유였는데, 사실과 반대되는 칭찬은 그 신뢰를
> 오히려 깎는다. ②③은 그대로 강제하고, ①에 「실제로 먹은 음식에서만 고르고 없으면 빼라」를 단다.
> **문장 수가 2~3문장이라 ①이 빠져도 형식이 무너지지 않는다.**
...
> **2026-07-30 개정 — `원재료성식품`도 적재한다**(<https://www.data.go.kr/data/15100065/standard.do>).
> **「사진에 찍히는 건 조리된 음식」이라는 전제가 틀렸다.** 실기동에서 과일 접시 사진의
> 「복숭아」가 `가공식품` 완전일치에 걸려 200g이 **450kcal**로 잡혔다 — 생복숭아는 80kcal이고,
> 매칭된 행은 말린 것으로 보이는 225kcal/100g짜리였다. 훑어보니 계통적이다:
>
> | 검색어 | 매칭 결과 | 실제 |
> | --- | --- | --- |
> | 사과 | 가공식품 26 | 52 |
...
> | 고구마 | 음식 「고구마전」 227 | 128 |
>
> 과일 접시처럼 **원재료 그대로 먹는 경우가 흔한데** 그 데이터셋이 통째로 비어 있어, 이름이
> 겹치는 가공품이나 다른 조리 음식이 대신 잡혔다. 「영양소는 식품DB에서 결정적으로」라는 이
> 도메인의 전제가 가장 흔한 음식군에서 성립하지 않고 있었다.
...
`Food.dataset`(`DISH`/`RAW`/`PROCESSED`)으로 출처를 구분한다. **한 테이블에 뭉쳐 같은 규칙으로 찾으면
...
`원재료성식품`도 같은 이유로 완전일치에만 참여한다.

**매칭 순서는 `음식` → `원재료` → `가공식품` 완전일치, 그다음 `음식` 부분일치다.**
`원재료`가 `가공식품`보다 앞인 이유는 위 개정에 적은 그대로다 — 이름이 정확히 같을 때
「말린 복숭아」보다 「복숭아」가 사진에 찍힌 것에 가깝다. 반대로 `음식`이 `원재료`보다 앞인 것은
유지한다: 이름이 같을 때는 조리된 쪽이 맞다(밥과 쌀처럼 조리 전후로 열량 밀도가 크게 다르다).
...
| `POST /diet/meals/{id}/retry` | 같이 503으로 거절한다 — **사용자가 요청한 재시도는 전부** |
| 끼니 피드백 (확정·수정에 따른 자동 생성) | 생성을 건너뛰고 `feedback = null`, `Meal.status = FAILED`. **점수는 살아 있다** |
| 하루 피드백 | 생성을 건너뛰고 `dayScore`만 반환 |
| 그 외 전부 | 정상 동작 — 프로필·확정·항목 수정·하루 집계·식품DB 검색은 LLM을 쓰지 않는다 |

**가르는 기준은 「사용자가 명시적으로 요청했는가」다.** 자동 생성은 조용히 건너뛰어도 되지만
(점수는 남고 끼니가 바뀌면 다시 만들어진다), 재시도 버튼을 누른 요청에 204를 돌려주면
아무것도 못 하면서 「접수됐다」고 거짓말하는 셈이다 — 상태가 `PENDING`으로 갔다가 곧 `FAILED`로
돌아오므로 사용자는 또 누른다.

> **2026-07-30 — `/meals/{id}/retry`가 이 표에서 빠져 있어 실제로 구현되지 않았다.**
> 위 줄이 인식 쪽 두 엔드포인트만 적어서, 짝인 피드백 재시도는 가드 없이 남았다. **표에 없는
> 것은 구현되지 않는다** — 이 세션에서만 같은 모양의 누락이 네 번 났다(주의 영양소 DTO,
> `ANALYSIS_IN_PROGRESS`, `servingSizeKnown`, 그리고 이것). 짝을 이루는 경로는 한 줄에
> 함께 적어야 한다.
...
- `DietScoreCalculatorTest` — KDRIs 범위 안이면 100점(경계값 50/65·10/20·15/30 포함),
  탄 75·단 8·지 17 → 88점, 밥만 → 40점, 순수 지방 → 0점(하한 고정), `macroKcal 0` → null,
