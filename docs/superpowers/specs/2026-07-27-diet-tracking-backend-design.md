...
4. **활동 에너지가 갱신됐다** — `DailyActivityService.upsert`가 그날 캐시 행을 지운다

③④가 없으면 남은 끼니의 `updatedAt`은 그대로라 ②가 걸리지 않는다. 그러면 **같은 응답 안에서
`dayScore`는 재계산돼 맞는데 `feedback` 문장은 지운 끼니를 그대로 언급한다.** 활동 에너지도
프롬프트에 들어가므로 같은 문제가 생긴다.
