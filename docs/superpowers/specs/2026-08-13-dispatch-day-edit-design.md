...
### `saveShifts`는 `slotCode`를 `null`로 눕힌다

같은 근거의 반대편이다. **사진이 원본이라면 사진에 없는 값도 남기면 안 된다.** 하루 편집으로 넣은 근무조가 그대로 남으면 `working: false`로 확정한 뒤에도 근무조가 붙어 있고, 그 행이 무인증 조회로 나간다.

지금은 `ShiftSaveDay`에 `slotCode`가 없으므로 `null`이 들어간다. **엄마 배차표 인식이 붙으면 이 줄이 `= day.slotCode`가 되는 자리다** — 줄을 미리 두는 것은 그때 새로 추가하는 것보다 놓칠 확률이 낮기 때문이다(`AGENTS.md`: "설계 문서에 적어 두는 것으로는 안 잡힌다").
