...
| 사용 가능 용량 | `end_battery_level - start_battery_level >= 40` | 짧은 보충은 분모가 작아 오차가 크다 |
...
**2026-08-17 실측:** 이 차량의 60개월 전 구간(2021-09~2026-08)을 실 DB에서 돌려 보니 `capacityKwh`가 null인 달이 하나도 없었다 — ΔSoC ≥ 40 조건을 채우는 충전이 매달 최소 1건씩은 있었다. 전체 누적으로도 range 표본 384건 대 capacity 표본 341건으로 11%밖에 차이가 나지 않는다. 「자주 null이다」는 예측은 이 차량의 실데이터와 맞지 않는다 — null 처리 자체는 표본이 더 적은 차량·기간을 위한 옳은 방어이므로 코드는 그대로 두되, null이 흔하다는 전제로 화면을 설계하지 않는다.
