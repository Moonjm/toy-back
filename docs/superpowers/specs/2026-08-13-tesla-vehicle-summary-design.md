...
#### 「열린」것만으로는 부족하다 — 최근 24시간 안이어야 한다

위 문단이 말한 「끝나지 않은 행」은 우선순위의 근거로만 쓰였고, **그 유령 자체를 거르는 조건이
빠져 있었다.** 실제 이 DB에는 마감되지 않은 충전 6건(2021~2025년)과 주행 12건(2022~2024년)이
쌓여 있었고, 조건 없는 `EXISTS`는 그중 하나만 있어도 늘 참이었다 —
**`state`가 배포 이후 한 번도 맞은 적이 없었다.** 늘 `charging`이었고, `states`의 진짜 값에는
도달조차 못 했다.

그래서 열린 행에 **`start_date >= (now() AT TIME ZONE 'UTC') - interval '24 hours'`**를 건다.

24시간인 근거: 완속 오버나이트 충전이 10시간쯤이고 한 번에 24시간 연속 주행은 없다. 진짜 세션은
이 창을 넘지 않고, **새로 생긴 유령도 하루면 스스로 낫는다.** 실제 유령이 전부 8개월 이상 된
데이터라 여유는 충분하다.

`charges`·`positions`에 최근 샘플이 들어왔는지 보는 편이 더 정밀하지만, `positions.drive_id`에
인덱스가 있는지 확인되지 않아 3,000만 행을 잘못 훑을 위험이 있다. 그 정밀도가 필요해지면
인덱스를 확인한 뒤에 옮긴다.
...
`cost IS NULL AND end_date IS NOT NULL`을 `start_date DESC`로, **최근 한 달 안의 것만** 낸다.
**기간 파라미터는 없다** — 범위가 고정이라 호출자가 고를 것이 없다.

한 달로 자르는 이유는, 오래된 것까지 다 내리면 채워 넣을 마음이 안 드는 길이가 되고 실제로
채우는 것은 최근 것이기 때문이다. 한 달 넘게 비워 둔 건은 앞으로도 채우지 않는다.

경계는 SQL에서 `(now() AT TIME ZONE 'UTC') - interval '1 month'`로 잡는다. Kotlin에서 `now()`를
쓰면 시계 주입과 테스트 이음새가 따라붙고, `start_date`가 타임존 없는 컬럼에 든 UTC 값이라
`now()`(timestamptz)와 그냥 비교하면 세션 타임존만큼(KST면 9시간) 창이 짧아진다.

`totalCount`는 `limit`과 무관하되 **같은 한 달 창 안의** 개수다 — 앱이 배지에 「미등록 3건」을
띄우고, 채울수록 줄어드는 것을 본다. **목록만 좁히면 배지와 목록이 어긋나** 무엇이 남았는지
읽히지 않는다. `limit` 기본값 50, 최대 200. 한 달 창이면 웬만해선 넘지 않지만, 더 필요하면
다시 부르면 된다(커서를 두지 않는다 — 채우면 목록에서 빠지므로 다시 부르면 다음 것들이 온다).

기존 목록 엔드포인트에 `costMissing=true` 필터를 얹지 않는다. 그쪽은 「기간 필수」가 계약인데
이쪽은 기간이 고정이다. 한 엔드포인트에 두 계약을 섞으면 「기간 없이 부르면 400」이 조건부가 된다.
