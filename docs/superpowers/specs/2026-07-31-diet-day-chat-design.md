...
```
[user]      [2026-08-03] 점심 왜 낮아?
[assistant] 나트륨이 기준을 넘었습니다...
[user]      [2026-08-05] 어제보다 나아졌어?
[assistant] 네, 12점 올랐습니다...
[user]      오늘은 어때?                    ← 이번 질문은 접두 없음
```

**붙이는 것은 `date`이지 `createdAt`이 아니다.** 「어느 날에 대한 질문인가」가 모델에게 필요한
값이다 — 8월 6일에 8월 1일을 물었다면 `[2026-08-01]`이다.

**연도까지 붙인다.** `date`는 창에 갇혀 있지 않아(`POST`가 상한 없이 과거 날짜를 받는다)
작년 같은 날이 이번 주 히스토리에 들어올 수 있다. 같은 이유로 `[직전 7일]` 블록도
연도를 찍는다 — 그쪽은 창이 7일이라 **해가 바뀌는 주에는 반드시** 두 해가 섞인다.
...
- **날짜를 넘나드는 히스토리가 들어간다** — 어제 물은 것이 오늘 대화의 컨텍스트에 실린다.
  「그날 것만」으로 되돌리면 빨개진다
- **히스토리의 사용자 턴에 날짜가 붙는다**(함정 5-1) — `[2026-08-03] 점심 왜 낮아?`. 붙는 값이
