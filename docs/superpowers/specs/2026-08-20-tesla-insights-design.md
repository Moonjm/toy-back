...
**한 줄 요약.** 앱의 통계 화면 하나를 채우는 `GET /tesla/insights`를 신설해 기존 `GET /tesla/drive-insights`를 흡수하고, 개요 화면의 배터리 추이를 내는 `GET /tesla/battery-window`를 따로 둔다. **`positions`(3,000만 행)를 훑는 계열은 배터리 추이 하나뿐이고, 나머지는 전부 `drives`·`charging_processes`만 읽는다.**
...
> `positions`는 3,000만 행에 `date`가 BRIN뿐이라 **범위 없는 `ORDER BY date DESC`가 11.7초**다 — 7일 범위를 먼저 돌리고 PK 역순으로 폴백한다.
...
**자정을 걸치는 세션의 분은 두 화면에서 다르게 다룬다.** 월별은 시작한 달로 몰아 친다 — 걸치는 것이 전 기간 충전 9건·주행 1건, 합쳐 3,220분(달당 약 0.12%)뿐이고, 쪼개면 `/tesla/summary`의 `trend`(역시 시작 시각 기준)와 같은 달에 다른 숫자를 내게 된다. 요일별 충전은 반대로 KST 자정으로 쪼개 넣는다 — 전 기간 충전 485건 중 310건(64%)이 자정을 걸쳐 시작 요일에 몰면 최근 12개월 기준 일요일 `idleMin`이 약 12% 과대가 된다. 요일별 주행은 걸치는 비율이 5년에 9건·127분뿐이라 쪼개지 않는다.

### SOC 추이 — 범위가 있으므로 괜찮다

48시간 범위는 BRIN이 잘 듣는다. 기존 `/tesla/status`가 7일 범위를 먼저 돌리는 것과 같은 형태다. **이것 하나만 `positions`를 읽는다.**

### 「최다 상승」은 뺀다

명예의 전당 후보 중 「최다 고도 상승」만 `positions.elevation`을 주행마다 훑어야 한다 — 범위가 없는 전 기간 스캔이다. **얻는 것이 타일 한 칸이라 값이 비용을 못 넘는다.** 최장거리·최장시간·최고효율 셋으로 간다.
...
- `positions`를 읽는 계열이 **범위가 있는 하나**로 국한된다.
...
| 6 | 48시간 SOC 범위 | **32ms · 12,517행**. 168시간은 102,141행 |
...
```jsonc
{
  ...
  // 이 범위 안의 충전 구간. 앱이 선 위에 다른 색으로 겹쳐 그린다.
  "charges": [{ "from": "2026-08-19T23:10:00", "to": "2026-08-20T02:40:00" }],

  // 최근 7일 팬텀 드레인. 범위(hours)와 무관한 고정 7일이다.
...
- **`parkDrain`이 범위를 안 따르는 이유:** 48시간 안에 순수 주차 구간이 하나도 없는 날이 흔하다. 「최근 7일」로 고정해야 숫자가 늘 나온다. `samples`가 0이면 앱이 그 줄을 감춘다.
