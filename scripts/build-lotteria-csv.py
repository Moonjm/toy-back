...
OUT_HEADER = (
    ...
    "transFatPer100g,cholesterolMgPer100g,maker,name"
)
KCAL_CARB = KCAL_PROTEIN = 4.0
KCAL_FAT = 9.0
...
def carb_energy_shares(food_csv):
    """음식DB 분류별 「탄수화물이 탄+지 열량에서 차지하는 비중」의 중앙값."""
    buckets = {}
    for row in csv.DictReader(open(food_csv, encoding="utf-8")):
        name = row["name"]
        if "_" not in name:
            continue
        carbs, protein, fat, kcal = (num(row[k]) for k in
                                     ("carbsPer100g", "proteinPer100g", "fatPer100g", "kcalPer100g"))
        if None in (carbs, protein, fat, kcal) or kcal <= 0:
            continue
        energy = KCAL_CARB * carbs + KCAL_FAT * fat
        if energy <= 0:
            continue
        # 아트워터가 안 맞는 행은 기준에서 뺀다 — 원본에도 자기모순인 행이 섞여 있다.
        if abs(KCAL_CARB * carbs + KCAL_PROTEIN * protein + KCAL_FAT * fat - kcal) > kcal * 0.15:
            continue
        buckets.setdefault(name.split("_")[0], []).append(KCAL_CARB * carbs / energy)
...
def split(kcal, protein, sugar, saturated, share):
    """잔여 열량을 탄수화물과 지방으로 나눈다. 물리 제약을 하한으로 건다.

    - **당류 ≤ 탄수화물** — 없으면 사이다(잔여 열량의 93%가 당류)에 없는 지방이 생긴다
    - **포화지방 ≤ 총지방**

    열량은 공식값을 그대로 지킨다. 두 하한이 동시에 잔여 열량을 넘으면 원본끼리 모순인
    것이라, 그때는 열량을 맞추는 쪽을 버리고 하한을 둘 다 지킨다(호출자가 경고를 남긴다).
    """
    rest = kcal - KCAL_PROTEIN * protein
    if rest <= 0:
        return 0.0, 0.0, rest < -0.5
    carbs = max(rest * share / KCAL_CARB, sugar)
    fat = (rest - KCAL_CARB * carbs) / KCAL_FAT
    if fat < saturated:
        fat = saturated
        carbs = (rest - KCAL_FAT * fat) / KCAL_CARB
        if carbs < sugar:
